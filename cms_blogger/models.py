...
class BlogEntryPage(getCMSContentModel(content_attr='content'),
    ...
    #   autogenerated draft or it will be '0' if the the entry has all
    #   required fields
    NOT_DRAFT = u'0'
    draft_id = models.CharField(db_index=True, max_length=32)

    objects = EntriesManager()

    def __init__(self, *args, **kwargs):
        super(BlogEntryPage, self).__init__(*args, **kwargs)
        self._initial_pub_date = self.publication_date
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_draft:
            ...
            # if other fields than slug&title are missing,
            #   it should not steal slug for other possible non-draft entries
            self.slug = ''
        else:
            self.draft_id = BlogEntryPage.NOT_DRAFT
...
class BlogCategory(models.Model, BlogRelatedPage):
    ...
    def get_entries(self):
        ...
        ).order_by(*BLOG_ENTRIES_ORDER_BY_UPDATE).distinct()
