...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        try:
            from cms_blogger.utils import user_display_name
            from cms_blogger.slug import get_unique_slug
        except (ImportError, ) as exc:
            print "Error: %s" % exc
            return

        Author = orm['cms_blogger.Author']

        def get_slug(author):
            display_name = author.name or user_display_name(author.user)
            authors = Author.objects.all()
            slug = get_unique_slug(author, display_name, authors)
            print "adding slug for author %s: %s" % (display_name, slug)
            return slug
        ...
        # empty slugs in order to regenerate them
        duplicated_authors.update(slug='')

        # fix empty slugs
        for author in Author.objects.filter(slug=''):
            author.slug = get_slug(author)
            author.save()
    ...
    models = {
        ...
        'cms.cmsplugin': {
            ...
            'creation_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2015, 6, 16, 0, 0)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'cms_blogger.blogentrypage': {
            ...
            'update_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now', 'blank': 'True'})
        },
